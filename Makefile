...
ifeq ($(TYPE)$(DUMP), LLVM1)
	${CC} -o ${EXECUTABLE} $(SRC_FILES) $(PARSER_CPP) $(SCANNER_CPP) ${CFLAGS} -I$(HEADERS_DIR) $(LLVM) -DLLVM -DLLVM_DUMP
else
ifeq ($(TYPE), LLVM)
	${CC} -o ${EXECUTABLE} $(SRC_FILES) $(PARSER_CPP) $(SCANNER_CPP) ${CFLAGS} -I$(HEADERS_DIR) $(LLVM) -DLLVM
else
	${CC} -o ${EXECUTABLE_TREE} $(SRC_FILES) $(PARSER_CPP) $(SCANNER_CPP) ${CFLAGS} -I$(HEADERS_DIR) $(LLVM)
endif
endif
	@echo "${GREEN}Creating executable... ${LIGHT_GREEN}Done!${WHITE}"
	@echo ''

superb: tmp parser scanner $(SRC_FILES) $(PARSER_CPP) $(SCANNER_CPP)
ifeq ($(DUMP), 1)
	${CC} -o ${EXECUTABLE} $(SRC_FILES) $(PARSER_CPP) $(SCANNER_CPP) ${CFLAGS} -I$(HEADERS_DIR) $(LLVM) -DLLVM -DLLVM_DUMP
else
	${CC} -o ${EXECUTABLE} $(SRC_FILES) $(PARSER_CPP) $(SCANNER_CPP) ${CFLAGS} -I$(HEADERS_DIR) $(LLVM) -DLLVM
